...
def getDict():
    ...
    dbConfig["db_args"]="-p zookeeperip=%%IP0%% -p queryip=%%IP1%% -p zookeeperport=2181 -p queryport=8090 -p replicants=5"
    dbConfig["db_name"]="druid_cl5_rf5"
    dbConfig["db_desc"]="Druid (Broker,Coordinator,Historical,MiddleManager,Overlord) on 5 VMs with Replication Factor 5. Ingest via Tranquility/Finagle, Query via REST."
