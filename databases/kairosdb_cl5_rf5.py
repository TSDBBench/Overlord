...
def getDict():
    ...
    dbConfig["db_client"]="kairosdb"
    dbConfig["db_args"]="-p ip=%%IP%% -p port=8080"
    dbConfig["db_name"]="kairosdb_cl5_rf5"
    dbConfig["db_desc"]="KairosDB with Cassandra on 5 VMs with Replication Factor 5."
    ...
    dbConfig["postrun"]= []
    dbConfig["prerun_master"]= ["%%SSH%%sudo -s bash -c 'sed -i \"s|kairosdb.datastore.cassandra.host_list=localhost:9160|kairosdb.datastore.cassandra.host_list=%%IP0%%:9160,%%IP1%%:9160,%%IP2%%:9160,%%IP3%%:9160,%%IP4%%:9160,|g\" /opt/kairosdb/conf/kairosdb.properties'",
                                "%%SSH%%sudo -s bash -c 'systemctl start cassandra.service'",
                                "%%SSH%%sudo -s bash -c 'sleep 60'",
                                "%%SSH%%sudo -s bash -c 'systemctl start kairosdb.service'",
                                "%%SSH%%sudo -s bash -c 'sleep 5'",
                                "%%SSH%%sudo -s bash -c 'cassandra-cli -h %%IP%% -f /home/vagrant/files/kairos_cassandra.cli'"]
    ...
    # check the same for kairos but only on master
    dbConfig["check_master"]= ["%%SSH%%sudo -s bash -c 'exit $(systemctl status kairosdb.service | grep -c \"active (exited)\")'",
                        "%%SSH%%sudo -s bash -c 'exit $(($(systemctl status kairosdb.service | grep -c \"active (running)\")-1))'",
