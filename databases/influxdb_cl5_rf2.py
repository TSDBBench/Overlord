...
def getDict():
    dbConfig={}
    dbConfig["db_folders"]=["/var/lib/influxdb"] # because /var/opt/influxdb/data does not exist at start (was: ["/var/opt/influxdb/data", "/var/opt/influxdb/wal"])
    dbConfig["db_client"]="influxdb"
    dbConfig["db_args"]="-p ip=%%IP%% -p port=8086"
    dbConfig["db_name"]="influxdb_cl5_rf2"
    dbConfig["db_desc"]="InfluxDB on 5 VMs with Replication Factor 2."
    ...
    dbConfig["prerun_dict"]= {
        0 : [
            ...
            "%%SSH%%sudo -s bash -c 'exit $(curl -G http://localhost:8086/query --data-urlencode \"q=ALTER RETENTION POLICY default ON testdb DURATION INF REPLICATION 2\" 2>&1 | grep -c \"error\")'",
            ]
    }
    ...
    dbConfig["check"]= ["%%SSH%%sudo -s bash -c 'exit $(systemctl status influxdb.service | grep -c \"active (exited)\")'",
                        ...
                        "%%SSH%%sudo -s bash -c 'exit $(($(/usr/bin/influx -execute \"SHOW SERVERS\"| grep -c \"true\")-"+
                        "$(/usr/bin/influx -execute \"SHOW SERVERS\"| grep -v \"true\" | grep -c \"false\")-1))'"]
