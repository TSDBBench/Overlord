...
def getDict():
    dbConfig={}
    dbConfig["db_folders"]=["/home/vagrant/h5serv/data"]
    dbConfig["db_client"]="h5serv"
    dbConfig["db_args"]="-p ip=%%IP%% -p port=5000"
    dbConfig["db_name"]="h5serv_cl1_rf1"
    dbConfig["db_desc"]="H5serv on 1 VM."
    ...
    dbConfig["check"]= []
    dbConfig["check_master"]= ["%%SSH%%sudo -s bash -c 'exit $(systemctl status h5serv.service | grep -c \"active (exited)\")'",
                        "%%SSH%%sudo -s bash -c 'exit $(($(systemctl status h5serv.service | grep -c \"active (running)\")-1))'"]
