...
def getDict():
    ...
    dbConfig["db_name"]="blueflood3"
    dbConfig["db_desc"]="Blueflood with Cassandra together on 5 VMs with Replication Factor 1."
    ...
    dbConfig["postrun"]= []
    dbConfig["prerun_master"]= ["%%SSH%%sudo -s bash -c 'sed -i \"s|CASSANDRA_HOSTS=localhost:9160|CASSANDRA_HOSTS=%%IP0%%:9160,%%IP1%%:9160,%%IP2%%:9160,%%IP3%%:9160,%%IP4%%:9160|g\" /home/vagrant/blueflood.conf'",
                                "%%SSH%%sudo -s bash -c 'sed -i \"s|localhost|%%IP0%%|g\" /home/vagrant/blueflood.conf'",
                                "%%SSH%%sudo -s bash -c 'systemctl start cassandra.service'",
                                "%%SSH%%sudo -s bash -c 'sleep 60'",
                                "%%SSH%%sudo -s bash -c 'cassandra-cli -h %%IP%% -f /home/vagrant/files/blieflood_cassandra.cli'",
    ...
    dbConfig["check_master"]= ["%%SSH%%sudo -s bash -c 'exit $(systemctl status blueflood.service | grep -c \"active (exited)\")'",
