...
def getDict():
    ...
    dbConfig["db_name"]="newts_cl1_rf1"
    dbConfig["db_desc"]="NewTS with Cassandra together on 1 VM."
    ...
    dbConfig["check_master"]= ["%%SSH%%sudo -s bash -c 'exit $(systemctl status cassandra.service | grep -c \"active (exited)\")'",
                        ...
                        "%%SSH%%sudo -s bash -c 'exit $(systemctl status newts.service | grep -c \"active (exited)\")'",
