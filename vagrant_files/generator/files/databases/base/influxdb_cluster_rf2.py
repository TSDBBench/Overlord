...
# this configures a influxdb cluster with replication factor 2
...
def getDict():
    ...
    baseConfig["prerun_dict"]= {
        4: [
            "%%SSH%%sudo -s bash -c 'exit $(curl -G http://%%HN0%%:8086/query --data-urlencode \"q=ALTER RETENTION POLICY default ON testdb DURATION INF REPLICATION 2\" 2>&1 | grep -c -i -E \"error|failed\")'"
        ],
    }
