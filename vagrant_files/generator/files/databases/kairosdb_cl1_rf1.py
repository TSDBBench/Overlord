...
def getDict():
    ...
    dbConfig["db_name"]="kairosdb_cl1_rf1"
    dbConfig["db_desc"]="KairosDB with Cassandra together on 1 VM."
    dbConfig["jvm_args"]="-jvm-args='-Xmx4096m'"
    dbConfig["prerun_once"]= []
    dbConfig["postrun_once"]= []
    dbConfig["prerun"]= []
    dbConfig["postrun"]= []
    dbConfig["prerun_master"]= ["%%SSH%%sudo -s bash -c 'cassandra-cli -h localhost -f /home/vagrant/files/kairos_cassandra.cli'"]
    ...
    dbConfig["check_master"]= ["%%SSH%%sudo -s bash -c 'exit $(systemctl status cassandra.service | grep -c \"active (exited)\")'",
                        ...
                        "%%SSH%%sudo -s bash -c 'exit $(systemctl status kairosdb.service | grep -c \"active (exited)\")'",
